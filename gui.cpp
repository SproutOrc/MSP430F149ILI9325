...
class GUI {
    ...
    void GUI_sprintf_string(uchar x, uint y,char *s, uint color,uint b_color);//显示英文字符串
    void GUI_sprintf_HZ1(uchar x, uint y, uint color,uint b_color);//显示预定义汉字字符串
    void GUI_sprintf_HZ2(uchar x, uint y, uint color,uint b_color);//显示预定义汉字字符串
    void GUI_sprintf_HZ3(uchar x, uint y, uint color,uint b_color);
...
GUI::void GUI_clearscreen(uint b_color)
{
	uint i,j;
...
**********************************************/
GUI::void GUI_Point(uchar x, uint y, uint color)
{  
	LCD_setxy(x,y);
...
以二进制形式显示出来。
************************************************************/
GUI::void GUI_sprintf_chartobit(uchar x, uint y,uchar dat, uint color,uint b_color)
{  
	uchar i ;
	for(i=0;i<8;i++)
	{
...
说明：无符号字符型数用指定的颜色和背景颜色在指定的坐标位置上
      以十六进制形式显示出来。
************************************************************/
GUI::void GUI_sprintf_chartohex(uchar x, uint y,uchar dat, uint color,uint b_color)
{  
//	int i ;
...
说明：无符号字符型数用指定的颜色和背景颜色在指定的坐标位置上
      以十进制形式显示出来。
************************************************************/
GUI::void GUI_sprintf_chartodec(uchar x,uint y,uchar dat,uint color,uint b_color)
{
...
说明：用指定位置上显示1位16进制数。
**********************************************/
GUI::void GUI_sprintf_nu(uchar x, uint y,uchar nu, uint color,uint b_color)
{  
...
************************************************************/
GUI::void GUI_HLine(uchar x, uint y, uchar length, uint color) 
{  
...
************************************************************/
GUI::void GUI_RLine(uchar x, uint y, uint high, uint color) 
{  
...
***********************************************************************/
GUI::void GUI_line(uint x1,uint y1,uint x2,uint y2,uint color)  
{  
    uint t;  
...
GUI::void GUI_DisPicture(uchar x, uint y, uchar length, uint high)
{
...
**********************************************/
GUI::void GUI_sprintf_char(uchar x, uint y,uchar c, uint color,uint b_color)
{  
	uchar  s_x ,s_y, temp ;
  	uint j;
  	c -= 32;
...
说明：在指定位置上显示英文字符串。
**********************************************/
GUI::void GUI_sprintf_string(uchar x, uint y,char *s, uint color,uint b_color)
{  
	for(;*s!='\0';s++)
	{
...
说明：在指定位置上显示预定义的中文字符串。
      要显示的中文字符串事先定义在chinese.h中的china_char[]数组中，
		...
		中文字符串自动转换为数组数据。
**********************************************/
GUI::void GUI_sprintf_HZ1(uchar x, uint y, uint color,uint b_color)
{ 
	uchar  s_x=0 ,s_y=0, temp=0 ;  
	uchar n;
	uint j,words;
...
GUI::void GUI_sprintf_HZ2(uchar x, uint y, uint color,uint b_color)
{ 
	uchar  s_x=0 ,s_y=0, temp=0 ;  
	uchar n;
	uint j,words;
...
{ 
	...
	uchar n;
	uint j,words;
	words=sizeof(chinese32x32)/128;
	for( n=0 ; n<words ; n++ )
  	{
	 	for( s_x=0 ; s_x<32 ; s_x++ ) 
	  	{
	   	if(s_x+x<240)
	   	{
	   	   j=n;
	   	   ...
	   	   temp = chinese32x32[j] ;
	   	   //temp = china_char[n*32+s_x] ;
	   		for( s_y=0 ; s_y<32 ; s_y++ )
	       	{
		     		if(y+s_y<320)
		       	{
			     		if((temp&(0x80>>(s_y))) == (0x80>>(s_y))  )
			       	{
				      	GUI_Point(x+s_x+n*32, y+s_y,b_color) ;
			       	}
			     		else
			       	{
				      	GUI_Point(x+s_x+n*32, y+s_y,color) ;
				   	}
					}
		   	}
		   }
	   	...
	   	if(s_x+x<240)
	   	{
		    	...
		    	for( s_y=0 ; s_y<32 ; s_y++ )
	       	{
		     		if(y+s_y<320)
		       	{
			     		if((temp&(0x80>>(s_y))) == (0x80>>(s_y))  )
			       	{
				      	GUI_Point(x+s_x+n*32, y+s_y+16,color) ;
			       	}
			     		else
			       	{
				      	GUI_Point(x+s_x+n*32, y+s_y+16,b_color) ;
				   	}
					}
		   	}
		   }
...
说明：在指定位置上画出实心矩形。
**********************************************/
GUI::void GUI_box(uchar sx,uint sy,uchar ex,uint ey,uint color)
{ 
	uint temp;   
...
说明：在指定位置上画出空心矩形。
**********************************************/
GUI::void GUI_rectangle( uchar sx,uint sy,uchar ex,uint ey,uint color)
{
...
注意：在使用该函数时，不论最终显示的数有几位，它都将在LCD屏幕上
      占10个字符的空间（显示的多位十进制数不足10位时，
		本函数会在有效显示数字前面补空格）。
**********************************************/
GUI::void GUI_wrul(uchar x, uint y, unsigned long num, uint color,uint b_color)
{
	unsigned long temp; 
	uchar i;
	for(i=0;i<10;i++)
...
**********************************************/
GUI::void GUI_wrlval(uchar x, uint y,unsigned long num,uchar bits,uchar dp,uint color,uint b_color)
{
...
**********************************************/
GUI::uint RGB(uchar R,uchar G,uchar B)
{
